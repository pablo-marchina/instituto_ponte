...
describe("parseTestToken - casos de borda", () => {
  beforeEach(() => {
    process.env.NODE_ENV = "test";
  });

  it("deve retornar null para token com prefixo invalido", async () => {
    const req = makeRequest({
      headers: { authorization: "Bearer invalid-prefix:uuid" },
    });
    await expect(auth.requireAuth(req, makeReply())).rejects.toThrow(
      "Token de autenticação inválido",
    );
...
describe("getSupabaseJwtSecret - sem env var", () => {
  beforeEach(() => {
    process.env.NODE_ENV = "production";
    delete process.env.AUTH_MODE;
  ...
  it("deve lancar erro de token invalido quando SECRET nao configurado (erro interno capturado)", async () => {
    const req = makeRequest({
...
describe("validateSupabaseJwt - erro com mensagem personalizada", () => {
  beforeEach(() => {
    process.env.NODE_ENV = "production";
    delete process.env.AUTH_MODE;
  ...
  it("deve propagar erro que comeca com 'Usuário'", async () => {
    mockJwtVerify.mockRejectedValue(new Error("Usuário não autorizado"));
    const req = makeRequest({
      headers: { authorization: "Bearer token-custom-error" },
    });
    await expect(auth.requireAuth(req, makeReply())).rejects.toThrow(
